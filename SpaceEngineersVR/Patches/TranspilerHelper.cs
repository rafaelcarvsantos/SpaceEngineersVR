...
public struct CodeInstructionIndex
{
	...
	public CodeGap before => new(index);
	public CodeGap after => new(index + 1);

	public CodeInstructionIndex previous => new(index - 1);
	public CodeInstructionIndex next => new(index + 1);
}
...
public struct CodeGap
{
	...
	public CodeInstructionIndex before => new(index - 1);
	public CodeInstructionIndex after => new(index);

	public CodeGap previous => new(index - 1);
	public CodeGap next => new(index + 1);
}
...
public class TranspilerHelper : IEnumerable<CodeInstruction>
{
	...
	public CodeRange All() => new(Begin(), instructions.Count);

	public CodeInstructionIndex First() => new(0);
	public CodeInstructionIndex Last() => new(instructions.Count - 1);

	public CodeGap Begin() => new(0);
	public CodeGap End() => new(instructions.Count);
	...
	public CodeRange Insert(CodeGap position, MoveLabels moveLabels, MoveBlocks moveBlocks, params CodeInstruction[] code)
	{
		...
		return new(position, code.Length);
	}
	...
	public CodeRange ReplaceRange(CodeInstructionIndex first, params CodeInstruction[] code)
	{
		...
		{
			Replace(new(first.index + i), code[i]);
		}
		return new(first, code.Length);
	}
	public CodeRange ReplaceRange(CodeGap begin, params CodeInstruction[] code)
	{
		for (int i = 0; i < code.Length; ++i)
		{
			Replace(new(begin.after.index + i), code[i]);
		}
		return new(First(), code.Length);
	}
	/*
	...
	*/
	...
	public bool InstructionsPassPredicates(CodeInstructionIndex index, params Predicate<CodeInstruction>[] predicates)
	{
		...
		{
			if (!predicates[i](this[new(index.index + i)]))
				return false;
		}
		return true;
	}
	...
	public CodeRange? FindFirst(CodeRange withinRange, params Predicate<CodeInstruction>[] predicates)
	{
		foreach (CodeInstructionIndex i in withinRange)
		{
			if (InstructionsPassPredicates(i, predicates))
				return new(i, predicates.Length);
		}
		return null;
	}
	...
	public IEnumerable<CodeRange> FindEach(CodeRange range, params Predicate<CodeInstruction>[] predicates)
	{
		...
		{
			if (InstructionsPassPredicates(i, predicates))
				yield return new(i, predicates.Length);
		}
	}
